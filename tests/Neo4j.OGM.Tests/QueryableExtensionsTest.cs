...
public class QueryableExtensionsTest
{
    ...
    [Fact]
    public async Task WhereNullResultTest()
    {
        var metadataMock = new Mock<MetaData>();
        var driverMock = new Mock<IDriver>();
        var entityGraphMapperMock = new Mock<IEntityMapper>();
        ...
        var result = await dbSet.Where(x => x.Name == value).ToListAsync();

        asyncTransactionMock.Verify(x => x.RunAsync(It.Is<Query>(y => y.Text == "MATCH (c:Person) WHERE (c.Name = $param_value_0) RETURN c")), Times.Once);
    ...
    [Fact]
    public async Task WhereContainsResultTest()
    {
        var metadataMock = new Mock<MetaData>();
        var driverMock = new Mock<IDriver>();
        var entityGraphMapperMock = new Mock<IEntityMapper>();
        ...
        var result = await dbSet.Where(x => x.Name.Contains(value)).ToListAsync();

        asyncTransactionMock.Verify(x => x.RunAsync(It.Is<Query>(y => y.Text == "MATCH (c:Person) WHERE (c.Name CONTAINS 'Test') RETURN c")), Times.Once);
